...
function selectedOption(selectId: string) {
  ...
  if (options.length != 1)
    throw "The number of selected options is not 1";
  return options[0];
}
...
function showCodepageDetail(codepoint: number) {
  ...
  getElementById('detail-category').textContent = `${getCharacterCategoryCode(codepoint)} (${getCharacterCategoryName(codepoint)})`;
  getElementById('detail-block').textContent = getBlockForCodepoint(codepoint).replace(/_/g, ` `);
  getElementById('detail-script').textContent = getScriptForCodepoint(codepoint).replace(/_/g, ` `);
  ...
  getElementById('detail-previous-cp').setAttribute('data-cp', itos(previousCodepoint(codepoint), 10));
